...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['add_collector'])) {
        // Require working credentials because collectors sign in through the landing page.
        $un = trim($_POST['username'] ?? '');
        $password = $_POST['password'] ?? '';
        if (preg_match('/^CT-/i', $un)) $un = 'CT-' . substr($un, 3);
        if ($un === '' || $password === '') {
            $_SESSION['collector_form_error'] = 'Username and password are required for collector sign-in.';
            header('Location: /disbasura/admin/collectors.php'); exit;
        }
        if (!preg_match('/^CT-[a-zA-Z0-9_]+$/', $un)) {
...
$collectorFormError = $_SESSION['collector_form_error'] ?? '';
unset($_SESSION['collector_form_error']);
...
<?php if ($collectorFormError): ?><div class="alert-error" style="margin-bottom:1rem"><?= htmlspecialchars($collectorFormError, ENT_QUOTES, 'UTF-8') ?></div><?php endif; ?>
<?php if($collectors): ?>
